...
Collecting for lumpsum payment later

*Installment loan*

Pay lump sum, get regular payments later
#+end_definition
...
Type shows available reflections at position

m - vertical/horizontal, g - glide reflection, 2 - half turn, 1 - translation
...
1. begin with a seed $s \in \mathbb{C}$

2. iterate $a_{n+1} = a_n^2 + s$
...
** Dimension
#+begin_definition
If the volume of an object X satisfies $\text{vol}(\alpha X) = \alpha^{d} \text{vol}(X)$

we call $d$ the *dimension* of X
#+end_definition

#+begin_examples
1. Let X be a sierpinsky gasket

   Since X is the union of 3 half-size copies of itself,

   $\text{vol}(X) = 3(\text{vol}(\frac{1}{2}X)) = 3 * \frac{1}{2}^{d}\text{vol}(X)$
...
2. Let X be a sponge (3D carpet)

   Since X is the union of 20 third-size copies of itself,

   $\text{vol}(X) = 20 \text{vol}(\frac{1}{3}X) = 20 \frac{1}{3}^d \text{vol}(X)$
...
* Statistical Data
** Census
- 850,000 people participated in the 2000 census
- census data used for district marking, resource allocation
- measurement problems include lack of centralized ID database, individual apathy, homeless individuals, illegal immigrants
** Sampling
#+begin_definition
...
Used to determine total populations.
