...
** Power basis
   ...
   $$\left[ \begin{matrix} x_1^n & \cdots & x_1 & 1 \\ x_2^n & \cdots & x_2 & 1 \\ \vdots  & \vdots & \vdots & \vdots \\ x_{n+1}^n & \cdots & x_{n+1} & 1 \end{matrix} \right] \left[ \begin{matrix} a_1 \\ a_2 \\ a_3 \\ a_3 \end{matrix} \right] = \left[ \begin{matrix} y_1 \\ y_2 \\ \vdots \\ y_{n+1}\end{matrix} \right]$$
...
** Lagrange basis
   ...
   Let $\psi(x) = \prod_{j = 1}^{n + 1} ( x - x_j)$

   $w_i = \frac{1}{\prod ^{j = 1,\ j = \neq i}$
